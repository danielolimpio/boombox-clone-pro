...
import { ChevronLeft, ChevronRight, MapPin, Sparkles, Clock, ListOrdered } from "lucide-react";
...
const TOTAL = fenomenosNaturais.length;
const SERIES_TITLE = "15 Fenômenos Naturais Impossíveis que a Ciência Ainda Não Consegue Explicar";
const SERIES_SUBTITLE = "De cataratas vermelho-sangue na Antártida a pedras que caminham sozinhas, conheça os mistérios da natureza que ainda desafiam a ciência";

const FenomenoNaturalPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const index = fenomenosNaturais.findIndex((f) => f.slug === slug);
  const item = index >= 0 ? fenomenosNaturais[index] : null;
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-16 text-center">
          <h1 className="text-2xl font-bold">Fenômeno não encontrado</h1>
          <Link to={`/fenomeno-natural/${fenomenosNaturais[0].slug}`} className="text-primary underline">
  ...
  const prev = index > 0 ? fenomenosNaturais[index - 1] : null;
  const next = index < TOTAL - 1 ? fenomenosNaturais[index + 1] : null;
  const showNewsletter = item.number === 8;
  const showTeaser = item.number === 10;
  const currentUrl = `https://efuxico.com.br/fenomeno-natural/${item.slug}`;
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <link rel="canonical" href={`https://efuxico.com.br/fenomeno-natural/${item.slug}`} />
        <meta property="og:type" content="article" />
        <meta property="og:title" content={item.metaTitle} />
        <meta property="og:description" content={item.metaDescription} />
        <meta property="og:url" content={`https://efuxico.com.br/fenomeno-natural/${item.slug}`} />
          ...
          <article className="lg:col-span-2">
            ...
            <figure className="mb-6">
              ...
              <figcaption className="text-xs text-muted-foreground mt-2 text-center italic">
                {item.imageAlt} • Fenômeno {item.number} de {TOTAL}
              </figcaption>
            </figure>
            ...
            <div className="bg-muted/40 border-l-4 border-primary rounded-r-lg p-4 mb-6 space-y-2">
              ...
              <p className="text-sm flex items-start gap-2">
                ...
                <span><strong>Mistério científico:</strong> {item.mystery}</span>
              </p>
            </div>
            ...
            <div className="bg-card border border-border rounded-lg p-4 mb-6 text-center">
              <p className="text-sm font-medium">
                💬 Você já tinha ouvido falar desse fenômeno? Comente nas suas redes sociais!
              </p>
            </div>
            ...
            {showTeaser && (
              <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-4 mb-6">
                <p className="text-sm font-semibold text-destructive">
                  ⚠️ O próximo fenômeno é tão estranho que parece ficção científica!
                </p>
              </div>
            )}
            ...
            <nav className="flex items-center justify-between gap-4 mt-8" aria-label="Navegação do artigo">
              {prev ? (
                <Button asChild size="lg" variant="outline" className="gap-2 h-14 px-6 text-base font-semibold flex-1">
                  <Link to={`/fenomeno-natural/${prev.slug}`}>
              ...
              {next ? (
                <Button asChild size="lg" className="gap-2 bg-primary hover:bg-primary/90 h-14 px-6 text-base font-semibold flex-1">
                  <Link to={`/fenomeno-natural/${next.slug}`}>
                    Próximo
            ...
            {!next && (
              <section className="mt-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Link to={`/fenomeno-natural/${fenomenosNaturais[0].slug}`} className="block p-4 border border-border rounded-lg hover:bg-muted transition">
                    <p className="text-xs text-primary font-semibold mb-1">CURIOSIDADES</p>
                    <p className="font-semibold text-sm">Reler do início: 15 Fenômenos Naturais Impossíveis</p>
                  </Link>
                </div>
              </section>
            )}
...
export default FenomenoNaturalPage;