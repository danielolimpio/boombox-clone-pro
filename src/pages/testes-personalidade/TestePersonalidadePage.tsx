...
import { ChevronLeft, ChevronRight, Brain, Sparkles, Clock, ListOrdered, Lightbulb, BookOpen, Target } from "lucide-react";
...
const TOTAL = testesPersonalidade.length;
const SERIES_TITLE = "15 Testes de Personalidade que Revelam Quem Você Realmente É";
const SERIES_SUBTITLE = "Do Rorschach ao Big Five: 15 testes psicológicos usados por especialistas para revelar traços profundos, motivações inconscientes e caminhos de autoconhecimento";

const TestePersonalidadePage = () => {
  const { slug } = useParams<{ slug: string }>();
  const index = testesPersonalidade.findIndex((t) => t.slug === slug);
  const item = index >= 0 ? testesPersonalidade[index] : null;
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-16 text-center">
          <h1 className="text-2xl font-bold">Teste de personalidade não encontrado</h1>
          <Link to={`/teste-personalidade/${testesPersonalidade[0].slug}`} className="text-primary underline">
  ...
  const prev = index > 0 ? testesPersonalidade[index - 1] : null;
  const next = index < TOTAL - 1 ? testesPersonalidade[index + 1] : null;
  const showNewsletter = item.number === 7;
  const showTeaser = item.number === 10;
  const currentUrl = `https://efuxico.com.br/teste-personalidade/${item.slug}`;
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <link rel="canonical" href={`https://efuxico.com.br/teste-personalidade/${item.slug}`} />
        <meta property="og:type" content="article" />
        <meta property="og:title" content={item.metaTitle} />
        <meta property="og:description" content={item.metaDescription} />
        <meta property="og:url" content={`https://efuxico.com.br/teste-personalidade/${item.slug}`} />
          ...
          <article className="lg:col-span-2">
            ...
            <figure className="mb-6">
              ...
              <figcaption className="text-xs text-muted-foreground mt-2 text-center italic">
                {item.imageAlt} • Teste {item.number} de {TOTAL}
              </figcaption>
            </figure>
            ...
            <div className="bg-muted/40 border-l-4 border-primary rounded-r-lg p-4 mb-6 space-y-2">
              ...
              <p className="text-sm flex items-start gap-2">
                ...
                <span><strong>Base científica:</strong> {item.base}</span>
              </p>
              <p className="text-sm flex items-start gap-2">
                <Target className="h-4 w-4 mt-0.5 text-primary shrink-0" />
                <span><strong>Aplicação:</strong> {item.application}</span>
              </p>
            </div>
            ...
            <div className="bg-primary/5 border border-primary/30 rounded-lg p-4 mb-6 flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-primary uppercase tracking-wider mb-1">Insight de autoconhecimento</p>
                <p className="text-sm text-foreground/90 italic">{item.insight}</p>
              </div>
            </div>

            <div className="bg-card border border-border rounded-lg p-4 mb-6 text-center">
              <p className="text-sm font-medium">
                💬 Você já fez esse teste? Compartilhe com quem ama autoconhecimento!
              </p>
            </div>
            ...
            {showTeaser && (
              <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-4 mb-6">
                <p className="text-sm font-semibold text-destructive">
                  ⚠️ Os próximos testes são os mais controversos da psicologia — e podem revelar aspectos que você prefere não enxergar!
                </p>
              </div>
            )}
            ...
            <nav className="flex items-center justify-between gap-4 mt-8" aria-label="Navegação do artigo">
              {prev ? (
                <Button asChild size="lg" variant="outline" className="gap-2 h-14 px-6 text-base font-semibold flex-1">
                  <Link to={`/teste-personalidade/${prev.slug}`}>
              ...
              {next ? (
                <Button asChild size="lg" className="gap-2 bg-primary hover:bg-primary/90 h-14 px-6 text-base font-semibold flex-1">
                  <Link to={`/teste-personalidade/${next.slug}`}>
                    Próximo
              ...
              <section className="mt-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Link to="/erro-historico/torre-de-pisa-erro-fundacao-arquitetura" className="block p-4 border border-border rounded-lg hover:bg-muted transition">
                    ...
                    <p className="font-semibold text-sm">10 Erros Históricos que Mudaram o Mundo Para Sempre</p>
                  </Link>
                  ...
                  <Link to={`/teste-personalidade/${testesPersonalidade[0].slug}`} className="block p-4 border border-border rounded-lg hover:bg-muted transition">
                    <p className="text-xs text-primary font-semibold mb-1">CURIOSIDADES</p>
                    <p className="font-semibold text-sm">Reler do início: 15 Testes de Personalidade</p>
                  </Link>
                </div>
              </section>
            )}
...
export default TestePersonalidadePage;